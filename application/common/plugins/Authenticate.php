...
namespace Karts\Application\Plugins;
...
//					}
		...
		if (!empty($methods)) {
			foreach ($methods as $action) {
				if (strpos($action, 'Action') !== false) {
					$actions[] = [
						'name'   => strtolower(str_replace('Action', '', $action)),
						'method' => $action
					];
				}
			}
	...
	{
		$permissions = \Model\Permissions::find(['order' => 'id ASC']);
		foreach ($permissions as $permission) {
			$this->permisions[strtolower($permission->module)][] = [
